...
class DataLoader:
    ...
    def get_gpt2(self):
        '''Returns GPT-2 representation of the dataset'''
        # Directory name for saving the datasets
        gpt2_dir = 'gpt2'

        def create_embedding(statement, tokenizer, model):
            '''Create a GPT-2 embedding from text'''
            sentences = tokenize.sent_tokenize(statement)
            vector = []
            
            for sentence in sentences:      
                # Encode tokens to prepare for applying weights
                try:
                    tokens = torch.tensor([tokenizer.encode(sentence)])
                except:
                    # Some characters aren't supported by the tokenizer
                    tokens = torch.tensor([tokenizer.encode(re.sub(r'[^\w]', ' ', sentence).replace("ʺ", "").replace("⅔", "two-third"))])

                # Predict hidden states features for each layer
                hidden_states, past = model(tokens)
                
                # Condense the matrix and add it to the total
                vector.append(hidden_states[0].flatten().tolist())
        ...
        def init():
            '''Initialize all logic from the main function'''
            # Check whether there is a file containing the GPT-2 data is already present
            if gpt2_dir in os.listdir(self.data_dir):
                return {
                    dataset: pd.read_pickle(os.path.join(
                        self.data_dir, gpt2_dir, dataset + '.pkl'))
                    for dataset in self.df.keys()
                }
            else:
                print('Creating GPT-2 representation and saving them as files...')
                ...
                for dataset in dfs:
                    ...
                    dfs[dataset]['statement'] = dfs[dataset]['statement'].progress_map(lambda text: create_embedding(text, tokenizer, model))

                # Create a storage directory
                os.mkdir(self.data_dir + '/' + gpt2_dir)
                
                # Save the datasets as pickle files
                for dataset in dfs.keys():
                    dfs[dataset].to_pickle(os.path.join(
                        self.data_dir, gpt2_dir, dataset + '.pkl'))
                print('Saved the datasets at ' + gpt2_dir)
